...
/*函数声明区域*/
//double atof(const char* s);  //VS中不用重复定义，存在于STDLIB.H
...
void clear(int n); /*清空Stack数组*/
int JudgeSymbol(int data, int mode);	/*mode=1:判定算式字符;mode=2:判定运算符,mode=3:判定数值；匹配成功返回对应下标，否则返回0*/
...
char str_symbol[9] = { "0+-*/^()" };     	/*运算符优先级设置，下标越大，优先级越高*/
